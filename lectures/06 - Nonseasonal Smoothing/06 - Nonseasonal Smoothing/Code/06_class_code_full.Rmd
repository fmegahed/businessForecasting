...
**We will discuss the plotting of the overall average at the beginning of next class**
...
usdt_comp = usdt %>% mutate(cAVG = cummean(adjusted), # fn from dplyr
                            ma3 = rollmeanr(adjusted, k = 3, na.pad = TRUE), # fn from zoo (loaded with tidyquant)
                            ma7 = rollmeanr(adjusted, k =7, na.pad = TRUE), # fn from zoo (loaded with tidyquant)
                            fcAVG = lag(cAVG), # base R fimcyopm
                            fma3 = lag(ma3),
                            fma7 = lag(ma7))

# The accuracy function is from forecast (which gets loaded with the fpp2 package)
# rbind just binds the rows (i.e. puts the results on top of each other)
...
round(results, digits = 6)
```


## Plotting of the best Model

**We will discuss this at the beginning of next class**
```{r bestModelPlot}
