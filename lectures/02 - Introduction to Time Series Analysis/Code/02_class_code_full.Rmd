...
```{r packages}
if(require(pacman)==FALSE) install.packages('pacman') # check and install if needed
pacman::p_load(tidyverse, COVID19, scales)
...
```{r getCOVIDdata, cache=TRUE}
statesCOVID = covid19(country = 'US',
                      level = 2,
                      start = '2020-03-01')

# Using the %in% to say if its either Ohio or Kentucky
df1 = statesCOVID %>% filter(administrative_area_level_2 %in% c('Ohio', 'Kentucky') )

# Alternatively you can use OR (|) 
df2 = statesCOVID %>% filter(administrative_area_level_2 == 'Ohio' |  administrative_area_level_2 == 'Kentucky')
```

## Plot the Data {.tabset .tabset-fade .tabset-pills}

### A Non-Paneled Line Plot {-}

```{r covidPlot1}
df1 %>% ggplot(aes(x = date, y = confirmed, group = id, color = administrative_area_level_2 )) +
  geom_line() +
  scale_y_continuous(labels = comma) + # makes the y-axis ticks more readable
  labs(x = 'Date', y = 'Total/Cumulative Cases', color = 'State' ) +
  theme_bw() + 
  theme(legend.position = 'top') # to change the location of the legend to top
```
...
```{r covidPlot2}
df1 %>% ggplot(aes(x = date, y = confirmed, group = id, color = administrative_area_level_2 )) +
  geom_line() + facet_wrap(~ administrative_area_level_2, ncol = 2, scales = 'free_y') +
  scale_y_continuous(labels = comma) + # makes the y-axis ticks more readable
  labs(x = 'Date', y = 'Total/Cumulative Cases', color = 'State' ) +
