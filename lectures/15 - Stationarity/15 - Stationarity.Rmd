...
### Quick Refresher on What we Covered in Chapter 06 so Far {.allowframebreaks}
...
\begin{block}{\textbf{Main Learning Outcomes Discussed in Chapter 06}}
  $\quad$ \textcolor{darkgreen}{\large \checkboxFadel} \textbf{Determine if a sample ACF plot “cuts off” or “dies down”.} \\
		...
		$\quad$ \textcolor{darkgreen}{\large \checkboxFadel} \textbf{Define the term “weakly stationary” and recognize time series that do not fit this definition.} \\
...
df %>% ggplot(aes(x= DATE, y = value, group = name, color = name)) + 
  geom_line() + facet_wrap(~ name, ncol = 1, scales = "free_y") + theme_bw() + theme(legend.position = 'bottom') + labs(x = 'Date') 
```
...
### Things to Do to Prepare for the Exam
 ...
 - Go over the interactive tutorial (will be posted by close of business 10-13-2020).  
