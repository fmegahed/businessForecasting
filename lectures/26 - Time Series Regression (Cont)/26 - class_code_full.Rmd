...
resplot(res = model4$residuals, fit = model4$fitted.values, freq = 4) # visualizing the residuals and fitted values
```

**Based on the J&J Example**, when our dependent and independent variables are observed over time, $\epsilon_t$  **is often correlated over time**. *In such cases, the assumptions of iid residuals are not met and the fitted regression models should not be used.*
...
autoplot(uschange[, c('Consumption', 'Income')], 
         facets = TRUE) + labs(y = "Pct Change by Quarter")# just plotting the data; argument facets = T creates the panels

reg1Fit = auto.arima(uschange[,'Consumption'], xreg = uschange[, 'Income'])
summary(reg1Fit)
```
...
**Investigating the residuals from the model:**
```{r inclassExample2}
resplot(res = reg1Fit$residuals, fit = reg1Fit$fitted, freq = 4)
checkresiduals(reg1Fit)
```
...
# In-Class Activities
  ...
  (1) Compare the results from the class example, with the following three models:  
    ...
    (b) `auto.arima()` using two explanatory variables ("Income" and "Savings").  
    (c) `lm` using both income and savings.  
    (d) `lm` using income only.
    
  (2) Which models are suitable? (i.e., the assumptions about the model residuals are met).  
  
  (3) Among the suitable models, pick the best model using the BIC. R function: `BIC()` from base R. 
  
  (4) **Bonus Activity:** Predicting 1-4 quarters ahead using the reg1 model (irrespective of whether it is the best model)


```{r inClassActivity}
# Solutions for the Questions
# (1a) Extrapolative forecasting using auto.arima (i.e., only the time-series for Consumption)
model1 = auto.arima(uschange[, 'Consumption'])
summary(model1)
...
model2 = auto.arima(uschange[,'Consumption'], xreg = uschange[, c('Income', 'Savings') ])
summary(model2)
resplot(res = model2$residuals, fit = model2$fitted, freq = 4)

# (1c) Lm using both income and savings 
model3 = lm(uschange[,'Consumption'] ~ uschange[,'Income'] + uschange[,'Savings'])
summary(model3)

# (1d) Income only
model4 = lm(uschange[,'Consumption'] ~ uschange[,'Income'] )
summary(model4)
resplot(model4$residuals, model4$fitted.values, freq = 4)


# Bonus Question: How to make predictions about future values
predict(reg1Fit, newxreg = c(0.1, -0.2)) # for reg1fit (one predictor)
# Alternatively 
forecast(reg1Fit, xreg = c(0.1, -0.2)) # for reg1fit (one predictor)
