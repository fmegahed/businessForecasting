...
Please go to \href{https://miamioh.instructure.com/courses/123532/quizzes/324283}{Canvas (click here)} and answer the question. **Due Sept. 14, 2020 [2:50 PM,
Ohio local time].** 
...
```{r cpi, echo=FALSE, eval=FALSE}
pacman::p_load(readxl, tidyverse, magrittr, fpp2)
download.file("https://www.wessexlearning.org/pobf2e/dsa/CPI_change_2.xlsx",
              destfile = "Data/CPI_change_2.xlsx", mode = "wb")

cpi = readxl::read_excel("Data/CPI_change_2.xlsx") %>% select(c(1,2))
cpi$observation_date %<>% ymd()
cpi %<>% filter(observation_date >= "2001-01-01")
cpi %<>% mutate(ma3 = rollmean(INFLATION, 3, align = "right", fill = NA))
cpi %<>% mutate(ma3 = lag(ma3)) 
accuracy(cpi$ma3, cpi$INFLATION)
```
